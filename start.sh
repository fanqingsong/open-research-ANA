...
# Open Research ANA 项目一键启动脚本
# 作者: AI Assistant
# 描述: 使用 Docker Compose 启动整个项目
...
print_header() {
    echo -e "${BLUE}================================${NC}"
    echo -e "${BLUE}  Open Research ANA 项目启动器${NC}"
    echo -e "${BLUE}================================${NC}"
}
...
check_env_file() {
    ...
    has_langsmith_key=$(grep -q "LANGSMITH_API_KEY=ls__" .env && echo "true" || echo "false")
...
show_status() {
    ...
    echo "  docker compose logs -f frontend # 查看前端服务日志"
    echo "  docker compose logs -f agent    # 查看 Agent 服务日志"
    
    echo ""
    print_message "停止服务命令:"
    echo "  docker compose down             # 停止所有服务"
    echo "  docker compose down -v          # 停止服务并删除数据卷"
}
